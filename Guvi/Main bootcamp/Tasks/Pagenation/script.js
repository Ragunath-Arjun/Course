...
if(startPage.innerHTML == 1){
    printName(0);
    startPage.removeAttribute('id','start-page');
...
function currentPage(num){
    //add active to current page and remove previouspage active
    if(num == JSONdetails.length/5)
    {
    ...
    previousPage();    
    const currentpage = document.getElementById('n'+num);
    currentpage.classList.add('active');
    const startvalue = ((num-1)*5);
    printName(startvalue);
}

function nextPage(){
    let activePage = document.querySelector('.active');
    let currentID = activePage.getAttribute('id');
    let arr = currentID.split("");
    let num = [];
    ...
    if(parseInt(num.join("")) < 20)
    {
        currentPage(parseInt(num.join(""))+1);
        const previous = document.getElementById('previous');
        previous.classList.remove('disabled');
    }
    else
...
function previousPage(){
    ...
    previouspage.classList.toggle('active');
    console.log(previouspage)
}

function printName(val){
    for (let index = 1; index <= 5; index++) {
        const spanName = document.getElementById('name'+index);
        const spanMail = document.getElementById('mail'+index);
        spanName.innerText = "Name: " + JSONdetails[val].name; 
        spanMail.innerText = "E-mail: " + JSONdetails[val].email;
        val++; 
    }
}