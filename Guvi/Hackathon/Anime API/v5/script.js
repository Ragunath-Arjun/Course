...
function createCard(data) {
    ...
    const p4 = backCardContent('p', "card-text", "Start Date: " + startyear);
    const p5 = backCardContent('p', "card-text", "End Date: " + endyear);
...
//Pagenation
async function pagenation(data, purl) {

    const pnav = document.createElement('nav');
    const pul = document.createElement('ul');
    pul.setAttribute('class', 'pagination justify-content-center')
    pnav.append(pul);
    document.body.append(pnav);

    let plength = data.last_page;
    for (let i = 1; i <= plength; i++) {
        let aurl = purl + "&page=" + (i);
        const li = document.createElement('li');
        li.setAttribute('class', 'page-item');
        const button = document.createElement('button');
        button.setAttribute('class', 'page-link');
        button.setAttribute('title', aurl);
        button.setAttribute('onclick', "pagenationdata(title)");
        button.innerText = i;
        li.append(button);
        pul.append(li);
        pnav.append(pul);
...
async function pagenationdata(purl) {
    let data = await fetch(purl);
    let fdata = await data.json();
    displayData(fdata.results);
}
